...
export default {
  ...
  data() {
    return {
      ...
      splitFileUrls: [],
      rootUrl:
        "http://218.249.32.238/files/encrypted/0b9/219511a7800d450da005d79e16460c3e_0_70969_encrypted_epub/OPS/",
      ebookMemu: [],
      dealwithArr: [],
      showMulu: false
    };
  ...
  methods: {
    ...
    authorzie() {
      this.$http
        .post("http://demo.kezhi.api.kingchannels.cn/aqr/authorize", {
          authorzieParameters: JSON.stringify({
            contentexternalid: "P00001-01-978-7-121-31649-4-Epub",
            organizationExternalId: "",
            isOnline: true,
            device: {
              devicekey: "qgJ9E46-mKww#Uzm",
              DeviceType: 4,
              Title: "电脑试读"
            },
    ...
    getList() {
      this.$http
        .get(
          "http://demo.bridge.kingchannels.cn/transfer/v1/api/tableofcontent/list",
          {
            params: {
              objectId: 93319,
              isRecursive: true,
    ...
    dealData(arrayBuffer, flag) {
      ...
      var key = CryptoJS.enc.Utf8.parse(
        this.makeKey("qgJ9E46-mKww#Uzm", this.Key)
      );
      // 解密流得到 wordarray
    ...
    getHtml(wordArray, flag) {
      ...
      xmlContent = xmlContent.substring(
        xmlContent.indexOf("<body>") + 6,
        xmlContent.indexOf("</body>")
      );
      ...
      xmlContent = xmlContent.replace(
        /<img([^>]+)src=\"[/]?([^"]+)\"([^>]*)>|<( *)img( *)[/>|>]/gi,
        '<img $1src="$2?width=400&quality=100&mode=min" $3 style="max-height:' +
          (window.innerHeight - 40) +
          'px">'
      );
      // 加载body计算分屏
      document.querySelector("#epub_content_box").innerHTML = xmlContent;
      document.querySelector("#epub_reader_wrap").style.columnWidth =
        ...
        if (imgDoms.length > 0) {
          for (let i = 0; i < imgDoms.length; i++) {
            // imgDoms[i].src = imgDoms[i].getAttribute('data-src');
              console.log(imgDoms[i].width)
            imgDoms[i].onload = function() {
              imgLoaded++;
              if (imgLoaded == imgDoms.length) {
                let iframeboxW = document.querySelector("#epub_reader_wrap")
                  .scrollWidth;
                let windowH = window.innerWidth;
                let pages = iframeboxW / windowH;
                _this.pageNum = pages;
                if (!flag) {
                  _this.pageIndex = _this.pageNum - 1;
                  document.getElementById(
                    "epub_content_box"
                  ).style.WebkitTransform =
                    "translateY(" +
                    -this.pageIndex * (window.innerHeight - 20) +
                    "px)";
                } else {
                  if (_this.hashId) {
                    let countPageIndex = parseInt(
                      document.getElementById(_this.hashId).offsetTop /
                        (window.innerHeight - 20)
                    );
                    if (_this.toUrlTimer) {
                      clearInterval(_this.toUrlTimer);
                      _this.toUrl = "";
                    }
                    let countTime = 1;
                    _this.toUrlTimer = setInterval(() => {
                      _this.pageIndex = parseInt(
                        document.getElementById(_this.hashId).offsetTop /
                          (window.innerHeight - 20)
                      );
                      if (_this.pageIndex == countPageIndex) {
                        document.getElementById(
                          "epub_content_box"
                        ).style.WebkitTransform =
                          "translateY(" +
                          -_this.pageIndex * (window.innerHeight - 20) +
                          "px)";
                        clearInterval(_this.toUrlTimer);
                        _this.toUrl = "";
                      } else {
                        countPageIndex = _this.pageIndex;
                      }
                    }, 100);
                  } else {
                    _this.pageIndex = 0;
          ...
          if (!flag) {
            ...
            document.getElementById("epub_content_box").style.WebkitTransform =
              "translateY(" +
              -this.pageIndex * (window.innerHeight - 20) +
              "px)";
          } else {
            if (this.hashId) {
              let countPageIndex = parseInt(
                document.getElementById(this.hashId).offsetTop /
                  (window.innerHeight - 20)
              );
              if (this.toUrlTimer) {
    ...
    next() {
      this.pageNum =
        document.querySelector("#epub_reader_wrap").scrollWidth /
        window.innerWidth;
      let epubContentBox = document.getElementById("epub_content_box");
      if (this.pageNum > this.pageIndex + 1) {
        this.pageIndex++;
        epubContentBox.style.WebkitTransform =
          "translateY(" + -this.pageIndex * (window.innerHeight - 20) + "px)";
      } else {
